    ...
    <tbody class="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-800">
        @forelse ($data as $row)
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition duration-150 ease-in-out">
                ...
                @if (!empty($actions))
                    <td class="px-6 py-4">
                        @if (in_array('business_show', $actions))
                            <x-admin.business.actions.show-action
                            :showModelColumns="$showModelColumns"
                            :row="$row"
                            />
                        @endif

                        @if (in_array('business_edit', $actions))
                            <x-admin.business.actions.edit-action
                                :row="$row"
                            />
                        @endif

                        @if (in_array('section_edit', $actions))
                            <x-admin.section.actions.edit-action
                                :row="$row"
                            />
                        @endif
